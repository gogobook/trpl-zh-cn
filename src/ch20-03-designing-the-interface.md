...
首先，讓我們探索一下為每一個連接都創建一個線程看起來如何。這並不是最終方案，因為正如之前講到的它會潛在的分配無限的線程，不過這是一個開始。示例 20-11 展示了 `main` 的改變，它在 `for` 循環中為每一個流分配了一個新線程進行處理：
...
<span class="caption">示例 20-11：為每一個流新建一個線程</span>
...
我們期望線程池以類似且熟悉的方式工作，以便從線程切換到線程池並不會對運行於線程池中的代碼做出較大的修改。示例 20-12 展示我們希望用來替換 `thread::spawn` 的 `ThreadPool` 結構體的假想接口：
...
<span class="caption">示例 20-12：如何使用我們將要實現的 `ThreadPool`</span>
...
繼續並對示例 20-12 中的 *src/main.rs* 做出修改，並利用編譯器錯誤來驅動開發。下面是我們得到的第一個錯誤：
