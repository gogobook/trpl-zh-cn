...
* 給定一系列數字，使用 vector 並返回這個示例的平均數（mean, average）、中位數（排列數組後位於中間的值）和眾數（mode，出現次數最多的值；這裡哈希函數會很有幫助）。
...
* 使用哈希 map 和 vector，創建一個文本接口來允許用戶向公司的部門中增加員工的名字。例如，「Add Sally to Engineering」 或 「Add Amir to Sales」。接著讓用戶抓取一個部門的所有員工的示例，或者公司每個部門的所有員工按照字母順排序的示例。
