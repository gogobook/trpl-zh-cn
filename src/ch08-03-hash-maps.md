...
最後介紹的常用集合類型是 **哈希 map**（*hash map*）。`HashMap<K, V>` 類型儲存了一個鍵類型 `K` 對應一個值類型 `V` 的映射。它通過一個 **哈希函數**（*hashing function*）來實現映射，決定如何將鍵和值放入內存中。很多編程語言支持這種數據結構，不過通常有不同的名字：哈希、map、對象、哈希表或者關聯數組，僅舉幾例。
