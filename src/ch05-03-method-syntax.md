...
讓我們將抓取一個 `Rectangle` 實例作為參數的 `area` 函數改寫成一個定義於 `Rectangle` 結構體上的 `area` 方法，如列表 5-13 所示：
...
讓我們更多的實踐一下方法，通過為 `Rectangle` 結構體實現第二個方法。這回，我們讓一個 `Rectangle` 的實例抓取另一個 `Rectangle` 實例並返回 `self` 能否完全包含第二個長方形，如果能返回 `true` 若不能則返回 `false`。一旦定義了 `can_hold` 方法，就可以運行列表 5-14 中的代碼了：
...
<span class="caption">列表 5-15：在 `Rectangle` 上實現 `can_hold` 方法，它抓取另一個 `Rectangle` 實例作為參數</span>
