...
這行代碼新建了一個叫做 `foo` 的變數並把它綁定到值 `bar` 上。在 Rust 中，變數默認是不可變的。下面的例子展示了如何在變數名前使用 `mut` 來使一個變數可變：
...
`&` 表示這個參數是一個 **引用**（*reference*），它允許多處代碼訪問同一處數據，而無需在內存中多次拷貝。引用是一個複雜的特性，Rust 的一個主要優勢就是安全而簡單的操縱引用。完成當前程序並不需要瞭解如此多細節：第四章會更全面的解釋引用。現在，我們只需知道它像變數一樣，默認是不可變的，需要寫成 `&mut guess` 而不是 `&guess` 來使其可變。
