...
結構體定義中的泛型類型參數並不總是與結構體方法簽名中使用的泛型是同一類型。列表 10-11 中在列表 10-8 中的結構體 `Point<T, U>` 上定義了一個方法 `mixup`。這個方法抓取另一個 `Point` 作為參數，而它可能與調用 `mixup` 的 `self` 是不同的 `Point` 類型。這個方法用 `self` 的 `Point` 類型的 `x` 值（類型 `T`）和參數的 `Point` 類型的 `y` 值（類型 `W`）來創建一個新 `Point` 類型的實例：
