...
例如，示例 11-20 有一個無意義的函數它打印出其參數的值並接著返回 10。接著還有一個會通過的測試和一個會失敗的測試：
...
<span class="caption">示例 11-10：一個調用了 `println!` 的函數的測試</span>
...
使用 `--nocapture` 參數再次運行示例 11-10 中的測試會顯示：
...
為了展示如何運行測試的子集，示例 11-11 使用 `add_two` 函數創建了三個測試來供我們選擇運行哪一個：
...
這運行了所有名字中帶有 `add` 的測試。同時注意測試所在的模組作為測試名稱的一部分，所以可以通過模組名來過濾運行一個模組中的所有測試。
