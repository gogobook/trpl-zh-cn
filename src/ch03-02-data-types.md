...
在 Rust 中，任何值都屬於一種明確的 **類型**（*type*），這告訴了 Rust 它被指定了何種數據，以便明確其處理方式。我們將分兩部分探討一些內建類型：標量（scalar）和復合（compound）。

Rust 是 **靜態類型**（*statically typed*）語言，也就是說在編譯時就必須知道所有變量的類型，這一認知將貫穿整個章節，請在頭腦中明確。通過值的形式及其使用方式，編譯器通常可以推斷出我們想要用的類型。多種類型均有可能時，比如第二章中使用 `parse` 將 `String` 轉換為數字時，必須增加類型註解，像這樣：
...
這些語句中的每個表達式使用了一個數學運算符並計算出了一個值，他們綁定到了一個變量。附錄 B 包含了一個 Rust 提供的所有運算符的列表。
...
**復合類型**（*Compound types*）可以將多個其他類型的值組合成一個類型。Rust 有兩個原生的復合類型：元組（tuple）和數組（array）。
...
`tup` 變量綁定了整個元組，因為元組被認為是一個單獨的復合元素。為了從元組中獲取單個的值，可以使用模式匹配（pattern matching）來解構（destructure）元組，像這樣：
...
程序首先創建了一個元組並綁定到 `tup` 變量上。接著使用了 `let` 和一個模式將 `tup` 分成了三個不同的變量，`x`、`y` 和 `z`。這叫做 *解構*（*destructuring*），因為它將一個元組拆成了三個部分。最後，程序打印出了 `y` 的值，也就是 `6.4`。
...
這個程序創建了一個元組，`x`，並接著使用索引為每個元素創建新變量。跟大多數編程語言一樣，元組的第一個索引值是 0。

#### 數組

另一個獲取一個多個值集合的方式是 **數組**（*array*）。與元組不同，數組中的每個元素的類型必須相同。Rust 中的數組與一些其他語言中的數組不同，因為 Rust 中的數組是固定長度的：一旦聲明，他們的長度不能增長或縮小。

Rust 中數組的值位於中括號中的逗號分隔的列表中：
...
數組在需要在棧（stack）而不是在堆（heap）上為數據分配空間時（第四章將討論棧與堆的更多內容），或者是想要確保總是有固定數量的元素時十分有用。雖然它並不如 vector 類型那麼靈活。vector 類型是標準庫提供的一個 **允許** 增長和縮小長度的類似數組的集合類型。當不確定是應該使用數組還是 vector 的時候，你可能應該使用 vector：第八章會詳細討論 vector。

一個你可能想要使用數組而不是 vector 的例子是當程序需要知道一年中月份的名字時。程序不大可能回去增加或減少月份，這時你可以使用數組因為我們知道它總是含有 12 個元素：
...
在這個例子中，叫做 `first` 的變量的值是 `1`，因為它是數組索引 `[0]` 的值。`second` 將會是數組索引 `[1]` 的值 `2`。

##### 無效的數組元素訪問

如果我們訪問數組結尾之後的元素會發生什麼呢？比如我們將上面的例子改為如下：
...
編譯並沒有產生任何錯誤，不過程序會導致一個 **運行時**（*runtime*）錯誤並且不會成功退出。當嘗試用索引訪問一個元素時，Rust 會檢查指定的索引是否小於數組的長度。如果索引超出了數組長度，Rust 會 *panic*，這是 Rust 中的術語，它用於程序因為錯誤而退出的情況。
