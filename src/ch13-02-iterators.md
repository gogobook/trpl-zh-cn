...
`next` 是 `Iterator` 實現者被要求定義的唯一方法。`next` 一次返回迭代器中的一個項，封裝在 `Some` 中，當迭代器結束時，它返回 `None`。如果你希望的話可以直接調用迭代器的 `next` 方法；代碼例 13-15 有一個測試展示了重複調用由 vector 創建的迭代器的 `next` 方法所得到的值：
