...
>
> 我稱之為我萬億美元的錯誤。當時，我在為一個面向物件語言設計第一個綜合性的面向引用的類型系統。我的目標是通過編譯器的自動檢查來保證所有引用的應有都應該是絕對安全的。不過我未能抗拒引入一個空引用的誘惑，僅僅是因為它是這麼的容易實現。這引發了無數錯誤、漏洞和系統崩潰，在之後的四十多年中造成了數以萬計美元的苦痛和傷害。
